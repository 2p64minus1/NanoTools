...
由于与系统资源管理器的神秘冲突，**在关闭转换后的输出文件（如 Word 或图片查看器）之前，请务必将本程序窗口挪动到该文件界面之外的位置**，否则程序可能会闪退。（没有怪windows的意思，估计是我程序问题）
...
为了稳定性，程序不会自动弹出文件夹。所有转换后的文件均保存在与**输入文件相同的目录下**。（例如图片原本的位置在C/download转换完后新图也在那个地方）
...
* **Video to GIF**: Hard-coded FFmpeg engine for maximum compatibility. | **视频转 GIF**: 内置 FFmpeg 引擎，极速（存疑）转换。
